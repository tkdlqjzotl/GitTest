...
namespace UnityEditor.MARS.Authoring
{
    ...
    class MarsCompareTool : EditorTool
    {
        ...
        const string k_Tooltip = "Select simulated data to compare it with this proxy's conditions.";

        class Styles
        {
            public readonly GUIContent stopComparingButtonContent = new GUIContent("Stop Comparing", "Stop comparing to simulated data.");
            public readonly GUIContent startComparingButtonContent = new GUIContent("Compare in Simulation View", k_Tooltip);
        ...
        public override GUIContent toolbarIcon => m_ToolbarIcon ?? (m_ToolbarIcon = new GUIContent(MarsUIResources.instance.CompareToolIcon, k_Tooltip));
