...
namespace UnityEditor.MARS.Build
{
    ...
    public abstract class ElectiveExtensions : IPreprocessBuildWithReport
    {
        ...
        public static string RunReport(SupportedConfiguration config)
        {
            ...
            {
                CheckUnityEditorVersion(config),
        ...
        static ElectiveReport CheckUnityEditorVersion(SupportedConfiguration config)
        {
            var report = new ElectiveReport();

            if (config.SupportedEditorVersions == null || config.SupportedEditorVersions.Length == 0)
            {
                return report;
            }

            if (config.SupportedEditorVersions.Any(version => EqualOrNewerVersion(Application.unityVersion, version)))
            {
                report.AddMessagePass(Messages.EditorVersionIsSupported + Application.unityVersion +
                                      Messages.EndQuoteNewLine);
                return report;
            }

            report.AddMessageFail(Messages.EditorVersionIsUnsupported +
                                  config.SupportedEditorVersions.Stringify() +
                                  Messages.DetectedVersionIs + Application.unityVersion + Messages.EndQuoteNewLine);
        ...
        static class Messages
        {
            ...
            public const string EditorVersionIsUnsupported =
                "Caution! The Unity Editor is not supported! \n    The supported Editor version is '";

            public const string EditorVersionIsSupported =
                "The Editor version is a match and is supported: '";
        ...
        public class SupportedConfiguration
        {
                ...
                ReportOutputLevels reportOutputLevel = ReportOutputLevels.WarnOnIssue, bool supportedGraphicsApIsIsOrdered = false)
            {
                ...
                SupportedEditorVersions = supportedEditorVersions;
