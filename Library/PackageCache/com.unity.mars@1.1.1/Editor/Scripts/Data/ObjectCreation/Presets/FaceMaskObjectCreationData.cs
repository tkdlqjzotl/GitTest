...
namespace UnityEditor.MARS
{
    class FaceMaskObjectCreationData : ObjectCreationData
    {
        ...
        static readonly Vector3 k_DepthMaskPositionRelativeToFaceMask = new Vector3(0, -0.153f, 0.08f);
        ...
        public override bool CreateGameObject(out GameObject createdObj, Transform parentTransform = null)
        {
            ...
            depthMaskGO.transform.position = k_DepthMaskPositionRelativeToFaceMask;
            //If Quaternion.Euler is used, the Facemask will show patches until rotation is re-set in the inspector
            depthMaskGO.transform.rotation = new Quaternion(0, 1, 0, 0);//Rot in Euler (0, 180, 0)
            depthMaskGO.transform.localScale = new Vector3(0.95f, 0.95f, 0.95f);
