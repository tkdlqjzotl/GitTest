...
namespace UnityEditor.MARS.Simulation
{
        ...
        IModuleDependency<SimulationVideoContextManager>, IProvidesDeviceSimulationSettings
    {
        ...
        const string k_EnvironmentModifiedDialogTittle = "Synthetic environment modified";
        ...
        const string k_NoModeSettingsName = "Custom (No Settings)";
...
#pragma warning restore 649
        ...
        public SimulationEnvironmentModeSettings CustomModeSettings => m_SimulationEnvironmentModeSettings;
        ...
        void RevertIgnoredModifications()
        {
            ...
            {
                var addedGo = addedGameObject.GetAssetObject() as GameObject;
        ...
        bool HasSaveableEnvironmentPrefabInstanceOverrides()
        {
        ...
        void OpenSyntheticEnvironment()
        {
            ...
            {
                envPrefabInstance = (GameObject)PrefabUtility.InstantiatePrefab(simulationSettings.EnvironmentPrefab,
            ...
            envPrefabInstance.name = simulationSettings.EnvironmentPrefab.name; // Removes "(clone)" from the name
        ...
        void OffsetEnvironment(Vector3 positionOffset, Quaternion rotationOffset, Vector3 scaleOffset)
        {
            ...
            foreach (var simView in SimulationView.SimulationViews)
            {
                ...
                simView.OffsetViewCachedData(environmentParentTrans, inversePreviousOffset, differenceRotation,
                    differenceScale);
        ...
        void UpdateEnvironmentModes()
        {
            s_ModeTypes = Enum.GetNames(typeof(EnvironmentMode))
                ...
                .Select(k => k.Replace(EnvironmentMode.Custom.ToString(), HasCustomModeSettings ? m_SimulationEnvironmentModeSettings.EnvironmentModeName : k_NoModeSettingsName))
        ...
        public void TryFrameAllSimViewsOnEnvironment()
        {
            ...
            foreach (var simView in SimulationView.SimulationViews)
            {
                ...
                FrameSimViewOnEnvironment(simView, SyntheticEnvironmentInfo, activeView, true);
            }
        }
        ...
        public Quaternion TryFrameSimViewOnEnvironment(ISimulationView simView, bool rotateView, bool instant)
        {
            ...
            return FrameSimViewOnEnvironment(simView, SyntheticEnvironmentInfo, rotateView, instant);
        }

        static Quaternion FrameSimViewOnEnvironment(ISimulationView simView,
            MARSEnvironmentInfo envInfo, bool rotateView, bool instant)
        {
            var session = MarsRuntimeUtils.GetMarsSessionInActiveScene();
        ...
        void TearDownEnvironmentNotCancelable()
        {
        ...
        bool TrySaveEnvironmentModificationsDialog(EnvironmentMode environmentMode, bool cancelable = true)
        {
                ...
                && CurrentLoadedPrefabInstanceEnvironment != null && HasSaveableEnvironmentPrefabInstanceOverrides())
            {
                ...
                {
                    if (EditorUtility.DisplayDialog(k_EnvironmentModifiedDialogTittle, k_EnvironmentModifiedDialogMessage,
                ...
                else
                {
                    var resultPopUpSaveModifiedEnvironment = EditorUtility.DisplayDialogComplex(
                        k_EnvironmentModifiedDialogTittle, k_EnvironmentModifiedDialogMessage,
