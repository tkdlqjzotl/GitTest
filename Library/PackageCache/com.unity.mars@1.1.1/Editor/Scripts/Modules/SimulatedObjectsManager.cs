        ...
        IModuleDependency<SceneWatchdogModule>
    {
        ...
        internal void CleanupSimulation()
        {
            ...
            if (SimulationSceneModule.isAssemblyReloading)
            {
                ...
                // This needs to happen before calls to CleanupBehavior because behaviors may clean up objects in OnDisable
                var logging = MarsDebugSettings.SimObjectsManagerLogging;
                ...
                foreach (var editor in Resources.FindObjectsOfTypeAll<Editor>())
                {
                    ...
                    foreach (var target in targets)
                    {
                        ...
                        // The list of all editors may contain a prefab asset, errors are printed to the console if its parent is set
                        if (PrefabUtility.IsPartOfPrefabAsset(targetGameObject))
                            continue;
        ...
        void StartSimulatableChangeTask()
        {
            ...
            {
                sleepTime = SimulationSettings.instance.TimeToFinalizeSceneModification,
