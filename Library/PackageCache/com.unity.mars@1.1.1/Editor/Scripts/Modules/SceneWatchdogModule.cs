...
using SlowTask = Unity.MARS.MARSUtils.SlowTaskModule.SlowTask;

namespace UnityEditor.MARS
{
    /// <summary>
    /// Polls the scene for changes to a MARS scene & provides callbacks when those changes happen
    /// </summary>
    ...
    public class SceneWatchdogModule : EditorScriptableSettings<SceneWatchdogModule>, IModuleDependency<SimulatedObjectsManager>
    {
        ...
        float m_PollingInterval = 1.5f;

        SimulatedObjectsManager m_SimulatedObjectsManager;

        SlowTask m_PollingTask;
        ...
        /// These have been destroyed & should only be used for the hash code or equality checks, like Dictionary removal
        ...
        void IModule.LoadModule()
        {
            ...
            m_PollingTask = new SlowTask
            {
                lastExecutionTime = Time.time,
                sleepTime = m_PollingInterval,
                task = ScenePoll
            };
        ...
        public void Update()
        {
            if (m_PollingTask != null)
                m_PollingTask.Update(Time.time);
        ...
        {
            m_PollingTask.task();
            m_PollingTask.lastExecutionTime = Time.time;
        }
