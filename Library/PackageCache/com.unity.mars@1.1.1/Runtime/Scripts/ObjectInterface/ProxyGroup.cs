...
namespace Unity.MARS
{
    ...
    public class ProxyGroup : MARSEntity, IUsesSetQueryResults, IHasEditorColor
    {
        ...
        /// </summary>
        public event System.Action<SetQueryResult> MatchChanged;
        ...
        void CallMemberOnUnmatchedEvents(bool seekNewMatch)
        {
            foreach (var kvp in m_ChildrenQueryArgs)
            {
                var memberProxy = kvp.Key;
                memberProxy.OnUnmatched?.Invoke(memberProxy.queryID, seekNewMatch);
            }
        }
        ...
        internal SetQueryArgs PerformQuery(QueryMatchID reservedID)
        {
            ...
            {
                Debug.LogWarning($"{name} must have at least 2 child Proxies for the group");
                return null;
            }
